...
int _write(int fd, void *buf, size_t count) {
  // _exit(SYS_write);
  // if(_syscall_(SYS_write, fd, buf, count) == 0){
  //   return count;
  // }
  // return 0;
...
#include "stdio.h"
extern char end;
static char buf[30] = "hello\n";
void *_sbrk(intptr_t increment) {
  static void *program_break = &end;
  if(_syscall_(SYS_brk, increment, 0, 0) == 0){
    void *last = program_break;
    intptr_t s = (intptr_t)program_break;
    s += increment;
    program_break = (void*)s;
...
int _gettimeofday(struct timeval *tv, struct timezone *tz) {
  ...
  // return 0;
  uint64_t time_us;
  time_us = _syscall_(SYS_gettimeofday, 0, 0, 0);
