...
//r: zxp, trans char to enum for keyboardevent!
#define enumdef(k) SDLK_##k,

enum class SDL_Keys_Send {
  SDLK_NONE = 0,
  _KEYS(enumdef)
};

// #define COND_ZXP(k) \
//   if (scancode == SDL_SCANCODE_##k) code = SDLK_##k;

//r: abort!! g++ dont support(start)
// #define arraydef(k) [SDL_SCANCODE_##k]=SDLK_##k,

// static uint8_t KEYCODEMAP[] = {
//   [SDL_SCANCODE_UNKNOWN]= SDLK_NONE,
//   _KEYS(arraydef)
//   0
// };
//r: (end)
//r: instead plan:
static uint8_t KEYCODEMAP[SDL_NUM_SCANCODES];

#define arrayInitDef(k) KEYCODEMAP[SDL_SCANCODE_##k] = uint8_t(SDL_Keys_Send::SDLK_##k);
//r: end
...
static const char* redirect_path(char *newpath, const char *path) {
  ...
  if (0 == access(newpath, 0)) {
    // fprintf(stderr, "File exist! Redirecting file open: %s -> %s\n", path, newpath);
    return newpath;
  }
  return path;
}
...
ssize_t read(int fd, void *buf, size_t count) {
  if (fd == dispinfo_fd) {
    ...
    if (has_key) {
      ...
      //r: trans standard sdl keycode to ysyx keycode!
      uint8_t ysyx_keycode = KEYCODEMAP[scancode];
      *(char*)buf = keydown ? 'd' : 'u';
      buf += sizeof(char);
      *(char*)buf = ysyx_keycode;
      return 2;
      // _KEYS(COND);
